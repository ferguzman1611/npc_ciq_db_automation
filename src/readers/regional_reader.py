"""
Reads the 'Regional Attributes' sheet and returns a list of dictionaries,
one per unique Region. Each dictionary holds the key-value pairs for that
region plus a 'region' key identifying it:
...
Uses the same label-boundary scan as global_reader, then groups the data
rows by region.
"""
...
def read_regional_attributes() -> list[dict]:
    ...
    data_start = start_row + 2  # skip label row and header row
    data_end = end_row - 1      # stop before the end label
